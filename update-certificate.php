...
if(isset($_POST['update_certificate'])) {
    ...
    // $path = $_POST['path'];
    $certificate_id = $_GET['certificate_id']; // Assuming project_id is provided in the URL
      ...
      if(isset($_FILES['pictures']) && !empty($_FILES['pictures']['name'][0])) {
            ...
            foreach($_FILES['pictures']['tmp_name'] as $key => $tmp_name) {
                ...
                $upload_directory = "../img/certificates/";
                ...
                if(move_uploaded_file($file_tmp, $upload_directory . $trimmed_directory)) {
                    ...
                    $image_path = $upload_directory . $trimmed_directory;
                        ...
                        $update_service_sql = "UPDATE certificates SET name='$name', date='$date', institution='$institute',path='$image_path'  WHERE id = $certificate_id";
    ...
    $update_service_sql = "UPDATE certificates SET name='$name', date='$date', institution='$institute'  WHERE id = $certificate_id";
    // Execute the insert query for image
        if($conn->query($update_service_sql) === FALSE) {
            echo "Error inserting image: " . $conn->error;
        }
...
<div class="container sp" id="takeNotesSection">
    ...
    <form id="noteForm" method="post" enctype="multipart/form-data">
        ...
        <label for="author">Date:</label>
          <input type="date" id="author" name="date" value="<?php echo isset($date) ? $date : ''; ?>">
          <span id="authorError" class="error"></span>
      
      
          <label for="author">Institute:</label>
          <input type="text" id="author" name="institute" value="<?php echo isset($institute) ? $institute : ''; ?>">
          <span id="authorError" class="error"></span>
        ...
        <span id="purposeError" class="error"></span></span>
          ...
          <!-- Image preview -->
          <img id="imagePreview" src="#" alt="Image Preview" style="display: none; max-width: 90px; max-height: 90px;">
        <br>  

        <input type="submit" name="update_certificate" value="Update certificate">
    </form>
